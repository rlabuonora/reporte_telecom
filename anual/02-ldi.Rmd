...
```{r}
serie_26 <- ldi %>% 
  group_by(year=year(fecha), servicio, sentido) %>% 
  summarize(val = sum(numeric, na.rm=TRUE))  %>% 
  filter(sentido == "desde", servicio == "Mn") %>% 
  filter(year > 2010) %>% 
  serie_historica_anual(val, nudge_text=c(10e6, 16e6, rep(10e6, 7)))
...
serie_27 <- ldi %>% 
  ...
  filter(minutos > 0) %>% 
  barras_horizontales(minutos, empresa, nudge_text = 6e6)
...
serie_29 <- ldi %>% 
  ...
  summarize(minutos = sum(numeric, na.rm=TRUE)) %>% 
  filter(minutos > 0) %>% 
  barras_horizontales(minutos, empresa, nudge_text = 2.5e6)
...
serie_30 <- ldi %>% 
  ...
  serie_historica_anual(val, nudge_text = 2e6)
...
serie_31 <- ldi %>% 
  ...
  summarize(minutos = sum(numeric, na.rm=TRUE)) %>% 
  filter(minutos > 0) %>% 
  pie_chart(minutos, empresa, nudge_x=c(0, 0) )
...
serie_32 <- ldi %>% 
  ...
  summarize(minutos = sum(numeric, na.rm=TRUE)) %>% 
  filter(!is.na(year), year > 2010) %>% 
  serie_historica_anual(minutos, nudge_text=2e6)
...
serie_33 <- ldi %>% 
  ...
  summarize(minutos = sum(numeric, na.rm=TRUE)) %>% 
  filter(!is.na(year), year > 2010) %>% 
  serie_historica_anual(minutos, 
                        nudge_text=c(rep(3e6, 2), 8e6, rep(3e6, 6)))
...
serie_34 <- ldi %>% 
  filter(red %in% c("Claro", "ANTEL", "Movistar"), sentido == "desde", servicio == "Mn") %>% 
  filter(year(FECHA_INFORME_ANUAL)==year(fecha)) %>% 
...
serie_40 <- ldi %>% 
  ...
  summarize(minutos = sum(numeric, na.rm=TRUE)) %>% 
  filter(!is.na(year), year > 2011) %>% 
  serie_historica_anual(minutos, nudge_text=1e6)
...
serie_42 <- ldi %>% 
  ...
  summarize(minutos = sum(numeric, na.rm=TRUE)) %>% 
  filter(!is.na(year), year > 2010) %>% 
  serie_historica_anual(minutos, nudge_text=2e5)
...
serie_43 <- ldi %>% 
  ...
  summarize(minutos = sum(numeric, na.rm=TRUE)) %>% 
  filter(!is.na(year), year > 2011) %>% 
  serie_historica_anual(minutos, nudge_text=6e6)
...
serie_45 <- ldi %>% 
  ...
  summarize(minutos = sum(numeric, na.rm=TRUE)) %>% 
  filter(!is.na(year), year > 2010) %>% 
  serie_historica_anual(minutos, nudge_text=3e6)
