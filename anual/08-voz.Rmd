...
## 4.70. Tráfico de voz de suscripciones móviles (minutos) por operador (`r FECHA_TITULO_FLUJO`)
```{r}
serie_151 <- voz %>% 
...
## 4.71. Tráfico de voz de suscripciones móviles (minutos) por destino (`r FECHA_TITULO_FLUJO`)
```{r}
serie_152 <- voz %>% 
  filter(year(fecha) == year(FECHA_INFORME_ANUAL)) %>% 
  group_by(tipo_red) %>% 
