...
serie_141 %>% 
  serie_historica_semestral_flujo(mensajes/1e6, nudge_text = 3e2) +
  labs(y="Millones")
```



## 124. Mensajes de texto enviados por operador (`r FECHA_TITULO_FLUJO`)
```{r}
serie_142 <- mensajes %>%
...
## 125. Mensajes de texto enviados según destino (`r FECHA_TITULO_FLUJO`)
```{r}
serie_143 <- mensajes %>%
  filter(year(fecha) == year(FECHA_INFORME_SEMESTRAL), 
         semester(fecha) == semester(FECHA_INFORME_SEMESTRAL)) %>% 
...
serie_144 %>% 
  serie_historica_semestral_flujo(mensajes/1e3, nudge_text = 1e4) +
  labs(y="Miles")
```
...
## 127. Mensajes de texto enviados al exterior por operador (`r FECHA_TITULO_FLUJO`)
```{r}
serie_145 <- mensajes %>% 
  filter(a == "Exterior") %>% 
  filter(year(fecha) == year(FECHA_INFORME_SEMESTRAL),
...
serie_146 %>% 
  serie_historica_semestral_flujo(mensajes/1e3, nudge_text = 2e5) +
  labs(y="Miles")
...
## 129. Mensajes de texto enviados a otras redes móviles por operador (`r FECHA_TITULO_FLUJO`)
