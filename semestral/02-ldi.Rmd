...
## 2.1. Minutos de tráfico de LDI salientes
...
## 2.4. Minutos de tráfico de LDI salientes de red fija - AMM por operador (`r FECHA_TITULO_FLUJO`)
...
## 2.6. Minutos de tráfico de LDI salientes de red fija - Resto País por operador (`r FECHA_TITULO_FLUJO`)
...
## 2.7. Minutos de tráfico de LDI salientes de Red Fija - Resto País
...
## 2.8. Minutos de tráfico de LDI salientes de redes móviles
...
## 2.27. Minutos de tráfico de LDI entrantes a Redes Móviles
...
## 2.28. Minutos de tráfico de LDI entrantes por red móvil destino (`r FECHA_TITULO_FLUJO`)
...
ldi %>%
  ...
  group_by(red) %>%
  summarize(minutos = sum(numeric, na.rm=TRUE)) %>%
  pie_chart(minutos, red, percent_accuracy = .01) %>%
  print
```
...
## 2.31. Minutos de tráfico de LDI entrantes a red móvil de Movistar
...
## 2.32. Minutos de tráfico entrante de LDI en red móvil de Telefónica por operador (`r FECHA_TITULO_FLUJO`)
```{r}
ldi %>%
  filter(red %in% c("Movistar"),
         sentido == "hacia",
...
## 2.33. Minutos de tráfico de LDI entrantes a red móvil de Claro
```{r}
ldi %>%
   filter(red %in% c("Claro"),
          sentido == "hacia",
