...
serie_112 %>% 
  serie_historica_semestral_stock(val / 1e3, nudge_text = 300) +
  labs(y="Miles")
```
...
serie_118 <-  servicios %>% 
  ...
  filter(year(fecha) >= 2016) %>% 
  group_by(year=year(fecha), semester=semester(fecha), month = month(fecha)) %>%
  # Error! faltaba esto:
  filter(month %in% c(6, 12)) %>% 
  summarize(val = sum(numeric, na.rm = TRUE))

serie_118 %>% 
  serie_historica_semestral_stock(val/1e3, nudge_text = 2e2) +
  labs(y="Miles")
```
...
serie_127 %>% 
  serie_historica_semestral_stock(val/1e3, nudge_text = 1e2) + 
  labs(y="Miles")
```
